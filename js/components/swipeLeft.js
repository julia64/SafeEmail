 ...
 *  基于eact-native-swipe-left
 *  增加了TEXT文本宽度设置textWidth以及图片高度imageWidth高度imageHeight设置
 */
                ...
                <View ref='view' style={[styles.containerBox, {backgroundColor:this.props.boxbgColor}]}>
                    ...
                    <Animated.View style={
                        ...
                        return <TouchableHighlight key={item.id} style={[styles.deletebtn, {width: item.width, height: this.state.height, backgroundColor: item.bgColor}]} onPress={this._onPress(item, rowId)} underlayColor={item.underlayColor}>
                                {item.text?
                                    <Text style={{color: item.color||null, fontSize: item.fontSize||null,width:item.textWidth||item.width,textAlign:'center'}}>{item.text}</Text>
                                    :
                                    item.image?
                                        <Image style={[styles.deleteBut, {width: item.imageWidth||50, height: item.iamgeHieght||50}]} source={{uri: item.image}} />
                                        :
                                        null}
                            </TouchableHighlight>
                        })}
